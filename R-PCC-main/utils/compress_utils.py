...
def compress_point_cloud(basic_compressor, plane_param, cluster_idx, salience_level, nonzero_residual_quantized,
    ...
    contour_map = contour_map.astype(bool)
...
def decompress_point_cloud(compressed_data, basic_compressor, model_num, H, W):
    ...
    # Assuming decompressed_data['plane_param'] is a bytes-like object
    plane_param_buffer = decompressed_data['plane_param']
    required_size = model_num * 4 * 4 # 4 * 4 is because each float32 occupies 4 bytes

    # print(len(decompressed_data['plane_param']))
    # print(model_num * 4 * 4) # 4 * 4 是因为每个 float32 占用 4 字节

    model_num = min(model_num, len(plane_param_buffer) // (4 * 4))
    if len(plane_param_buffer) >= required_size:
        plane_param = np.frombuffer(plane_param_buffer, dtype=np.float32, count=model_num * 4)
        plane_param = plane_param.reshape((model_num, 4))

    else:
    # Handle case where buffer size is insufficient
        print(f"Error: Buffer size {len(plane_param_buffer)} is insufficient for requested array of size {required_size}")
