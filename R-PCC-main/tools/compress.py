...
def compress():
    ...
    def compress_one_bin(input_file):
        output_file=input_file.replace(input_file.split('.')[-1],'rpcc')
        output_file=output_file.replace('input','output')
        pc_seg = PointCloudSegment(dataset.transform_map)
        if not args.cpu:
            # for time cost
            point_cloud, range_image, original_point_cloud = dataset.load_range_image_points_from_file(input_file)
        ...
        save_compressed_bitstream(output_file, compressed_data, uniform=uniform)
        t_save = time.time()


        print('\nCompression finished.')
        print('binary bitstream save in ', output_file)
        ...
        # compressed_bit_size = os.path.getsize(output_file) * 8
        # print('\nCompression Results: ')
        ...
        # if args.eval:
        #     # reconstruct
        #     compressed_data = read_compressed_bitstream(output_file, uniform=uniform)
    ...
    class Subscriber(Node):
        ...
        def __init__(self):
            super().__init__('bin_receive')
            self.bin_path=None
            self.rpcc_path=None
            self.subscription = self.create_subscription(
                        String,
                        'bin_path', # 替换为实际的ROS 2话题名称
                        self.bin_callback,
                        10 # 接收队列大小
                        )
            
            #传输rpcc文件路径
            self.publisher= self.create_publisher(String, 'rpcc_path', 10)  
            timer_period = 0.2 # seconds  
            self.timer1 = self.create_timer(timer_period, self.timer_callback)

        def timer_callback(self):  
            if(self.rpcc_path!=None):
                msg = String()  
                msg.data = (str)(self.rpcc_path)
                self.publisher.publish(msg)  
                self.get_logger().info('Publishing: "%s"' % msg.data) 
            
        def bin_callback(self,msg):  
            if(msg.data!=None):
                self.rpcc_path=compress_one_bin(msg.data)
                # self.timer_callback()
                self.get_logger().info(f'Received:{self.rpcc_path}')
